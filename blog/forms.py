...
class PostForm(forms.ModelForm):
    class Meta:
        model = Post
        fields = ['title', 'category', 'status', 'tags', 'content', 'featured_image']
        widgets = {
            ...
            'status': forms.Select(attrs={'class': 'w-full px-4 py-2 rounded-xl bg-white/90 border border-white/30 text-gray-800'}),
            ...
            'featured_image': forms.ClearableFileInput(attrs={'class': 'block w-full text-sm text-gray-600', 'accept': 'image/*'}),
        }
